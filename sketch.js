...
function setup(){
  ...
  // creating trex
  trex=createSprite(50,height-50,10,50);
  ...
  r=Math.round(random(100,width/2));
  console.log(r);
  ...
  gameOver=createSprite(width/2,(height/2)-100,25,25);
...
function draw(){
  ...
  if(gameState=="PLAY"){
    ...
    if(touches.length>0 || keyDown("space") && trex.y>height/2+400) {
      trex.velocityY=-15;
      jumpSound.play()
      touches=[];
    }
...
function spawnClouds() {
  if(frameCount%60==0) {
    cloud=createSprite(width,r,10,10);
    cloud.velocityX=-5;
    cloud.addImage(cloudImage);
    cloud.lifetime=width/cloud.velocityX+100;
...
function spawnCactai() {
  if(frameCount%60==0) {
    ...
    cactus.scale=0.5;
    cactus.lifetime=width/cactus.velocityX+100;
